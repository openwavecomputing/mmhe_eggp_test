...
class cancelledList extends Component {
  ...
  componentDidMount() {
    this.props.dispatch({ type: 'headerTitle', value: "My Application : Cancelled List" })
    this.GetListData('Cancelled')
  }
  ...
  viewRequest = (Referenceno) =>{
    ...
    window.location = process.env.REACT_APP_ENV +"/cancelled/view?p=" + encodeURI(CryptoJS.AES.encrypt(JSON.stringify(Referenceno), process.env.REACT_APP_ENCRYPT_PASSWORD).toString());
  }

  workflowRequest = (Referenceno) =>{
    var CryptoJS = require("crypto-js");
    window.location = process.env.REACT_APP_ENV +"/cancelled/workflow?p=" + encodeURI(CryptoJS.AES.encrypt(JSON.stringify(Referenceno), process.env.REACT_APP_ENCRYPT_PASSWORD).toString());
  }
  ...
  printRequest = (Referenceno) =>{
    var CryptoJS = require("crypto-js");
    window.location = process.env.REACT_APP_ENV +"/cancelled/print?p=" + encodeURI(CryptoJS.AES.encrypt(JSON.stringify(Referenceno), process.env.REACT_APP_ENCRYPT_PASSWORD).toString());
  }
       ...
       },
       {
        ...
        options: {
         ...
         sort: true,
          customBodyRender: (value, tableMeta, updateValue) => {
            return (
              <div className="badge badge-cancelled">{value}</div>
            );
          }
        }
       },
    ...
    return (
       <>
        <Breadcrumb>
          ...
          <Breadcrumb.Item active>My Application : Cancelled List</Breadcrumb.Item>
        </Breadcrumb>
        <MUIDataTable
          title={""}
          data={dataArray}
...
export default connect(mapStateToProps)(cancelledList);