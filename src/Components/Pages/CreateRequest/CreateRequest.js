...
class CreateRequest extends Component {
  ...
  componentDidMount() {
    ...
    this.props.dispatch({ type: "headerTitle", value: "Create Request" });
  ...
  //  console.log("testtsadasdgajh")
    //console.log(hostaddress)

    // let acname = {  "eslintConfig": {
    //     "globals": {
    //       "window": true,
    //       "ActiveXObject": true
    //     }
    //   } }
    //     var network = new acname('WScript.Network');
  ...
  GetGoodsItemsListData = () =>{
    // alert('a')
    //alert(this.props.Referenceno)
    let postData = { Referenceno: this.props.Referenceno }
  ...
  handleChange = (e) => {
    ...
    switch (e.target.name) {
      ...
      case "Authorizedcompanyaddress":
        ...
        break;
        case "searchcompanyno":
          this.setState({SearchCompanyNo:e.target.value})
        break;
      case "Vehiclenumber":
  ...
  searchCompanynotry = () => {
    ...
    .then(data => {
      console.log(data)
      if(data){
        this.setState({ companynolistArray: data.CompanyList })
  ...
  saveRequesterInformation =() =>{
    ...
    fetch(process.env.REACT_APP_WEB_SERVICE_URL + '/api/GGP/InsertRequesterInfo', {
    method: 'POST',
    headers: {
      'Content-Type': 'application/json',
      'Authorization': 'Bearer ' + this.props.getToken,
    ...
    .then(data => {
      //console.log("Save Requester Information")
      //console.log(data)
      if(data){
        this.props.dispatch({ type: 'Referenceno', value: data.Referenceno })
        this.props.dispatch({ type: "headerTitle", value: ((data.Referenceno) ? "Create Request - " + data.Referenceno : "Create Request")});
        this.setState({ Referenceno: data.Referenceno, isRequesterInformation: "yes" })
      }
    })
  }
          ...
          if (this.validator3.allValid()) {
            if (this.state.GoodsItemslistArray.length >= 1) {
              ...
              if(this.state.AttachmentslistArray.length > 0 ){
                ...
                window.location = process.env.REACT_APP_ENV +"/create/approverlist?p=" + encodeURI(CryptoJS.AES.encrypt(JSON.stringify(this.props.Referenceno), process.env.REACT_APP_ENCRYPT_PASSWORD).toString());
              }
            }else{
  ...
  onDrop = (acceptedFiles) => {
    ...
    acceptedFiles.map(file => {
      ...
      data.append('ReferenceNo', this.props.Referenceno);
  ...
  companysearchnumb =(val) =>{
    this.setState({SearchCompanyNo :val})
    this.state.companynolistArray.map((option) => {
      if(option.CompanyNo === val){
        this.setState({Authorizedcompanyname : option.CompanyName})
      }
    })    
  }
    ...
    return (
      <>
        <Breadcrumb>
          ...
          <Breadcrumb.Item active>Create Request</Breadcrumb.Item>
        </Breadcrumb>
        <ul id="progressbar">
            ...
            <div className="form-group row">
              ...
              <div className="col-sm-4">
                <div className="input-group mb-2 mr-sm-2">
                  ...
                  <div className="input-group-prepend"  onClick={this.showFirstStaffPopup}>
                    <div className="input-group-text">
                      <i className="fa fa-search text-muted"> </i>
                    </div>
                  </div>
                  <div className="input-group-prepend ml-3" style={{display: this.state.canStafNamePopover }}>
                    <button
                      id="PopoverLegacy"
                ...
                return (
                  <>
                    ...
                    <div className="form-group row">
                      ...
                      <Autocomplete
                      value={this.state.SearchCompanyNo}
                      name="searchcompanyno"
                      id=""
                        disablePortal
                        disableClearable
                        options={this.state.companynolistArray.map((option) =>option.CompanyNo)}
                        renderInput={(params) => <TextField {...params} margin="normal" label="" variant="outlined" />}
                        onChange={(event,compvalue) => {
                          this.companysearchnumb(compvalue)
                        }}
                      />
                        {
                          this.validator2_2.message('Company No', this.state.SearchCompanyNo, 'required')
                        }          
                      </div>
                      <div className="col-sm-4">
                        <div className="input-group mb-2 mr-sm-2" >
                          <input type="text" className="form-control" placeholder="" readOnly={true} name="Authorizedcompany" value={this.state.Authorizedcompanyname} />                         
                        </div>
                        {
                          this.validator2_2.message('Authorized Company Name', this.state.Authorizedcompanyname, 'required')
                        }
                      </div>
                    </div>
                    <div className="form-group row">
      ...
      <div className="text-center text-sm-right">
          ...
          <div className="table-responsive">
            <table className="table table-hover">
              ...
              <div className="text-danger text-center" style={{display: this.state.showGoodsItemRequired }}>The Goods Item is required.</div>
            </div>
            <div className="text-center text-sm-right">
    ...
    <table className="table table-hover">
      ...
      <tbody style={{display: (this.state.AttachmentsLoader === "Yes" ? "none" : "") }}>
       ...
       {(() => {
        ...
        <p className="text-center" style={{display: (this.state.AttachmentsLoader === "Yes" ? "" : "none") }}><Spinner animation="border" variant="primary" className="text-center" /></p>
         <div className="text-danger text-center" style={{display: this.state.showAttachedRequired }}>The Attachments is required.</div>
      </div>
      ...
      <table className="table table-hover">
        ...
        <p className="text-center" style={{display: (this.state.ReferencesLoader === "Yes" ? "" : "none") }}><Spinner animation="border" variant="primary" className="text-center" /></p>
         <div className="text-danger text-center" style={{display: this.state.showReferenceRequired }}>The References is required.</div>
        </div>
        <div className="text-center text-sm-right">
          ...
          <Modal.Body>
            ...
            <section> 
              ...
              <div className="table-responsive max-height-280"> 
                ...
                <p className="text-center"><Spinner animation="border" variant="primary" style={{display: this.state.StaffDetailslistArraySpin }} className="text-center" /></p>
              </div>
            </section>
          </Modal.Body>  
          <Modal.Footer>  
