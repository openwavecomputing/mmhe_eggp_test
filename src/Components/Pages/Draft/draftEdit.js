...
class CreateRequest extends Component {
  ...
  componentDidMount() {
    ...
    this.props.dispatch({ type: "headerTitle", value: ((this.state.Referenceno) ? "Draft : Edit - " + this.state.Referenceno : "Draft : Edit")});
          ...
          if (this.validator3.allValid()) {
            if (this.state.GoodsItemslistArray.length >= 1) {
              ...
              if(this.state.AttachmentslistArray.length > 0){
                ...
                window.location = process.env.REACT_APP_ENV +"/draft/editapproverlist?p=" + encodeURI(CryptoJS.AES.encrypt(JSON.stringify(this.state.Referenceno), process.env.REACT_APP_ENCRYPT_PASSWORD).toString());
              }
            }else{
    ...
    return (
      <>
        <Breadcrumb>
          ...
          <Breadcrumb.Item href={process.env.REACT_APP_ENV +"/draft/list"}>My Application : Draft List</Breadcrumb.Item>
