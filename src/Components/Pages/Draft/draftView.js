...
class draftView extends Component {
  ...
  componentDidMount() {
    ...
    this.props.dispatch({ type: 'headerTitle', value: "Draft : View" })
    this.getGGPRequest();
  }
  ...
  editFloatingIcon = () => {
    var CryptoJS = require("crypto-js");
    window.location = process.env.REACT_APP_ENV + "/draft/edit?p=" + encodeURI(CryptoJS.AES.encrypt(JSON.stringify(this.state.Referenceno), process.env.REACT_APP_ENCRYPT_PASSWORD).toString());
  }
  render() {
    if(this.state.loading){
        return (
            ...
            <Breadcrumb.Item href={process.env.REACT_APP_ENV +"/draft/list"}>Draft : View</Breadcrumb.Item>
