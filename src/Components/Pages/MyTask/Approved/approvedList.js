...
class approvedList extends Component {
  ...
  componentDidMount() {
    this.props.dispatch({ type: 'headerTitle', value: "My Task : Approved List" })
  ...
  viewRequest = (Referenceno) =>{
    ...
    window.location = process.env.REACT_APP_ENV +"/mytask/approved/view?p=" + encodeURI(CryptoJS.AES.encrypt(JSON.stringify(Referenceno), process.env.REACT_APP_ENCRYPT_PASSWORD).toString());
  }

  workflowRequest = (Referenceno) =>{
    var CryptoJS = require("crypto-js");
    window.location = process.env.REACT_APP_ENV +"/mytask/approved/workflow?p=" + encodeURI(CryptoJS.AES.encrypt(JSON.stringify(Referenceno), process.env.REACT_APP_ENCRYPT_PASSWORD).toString());
  }
  ...
  printRequest = (Referenceno) =>{
    var CryptoJS = require("crypto-js");
    window.location = process.env.REACT_APP_ENV +"/mytask/approved/print?p=" + encodeURI(CryptoJS.AES.encrypt(JSON.stringify(Referenceno), process.env.REACT_APP_ENCRYPT_PASSWORD).toString());
  }

  render() {
    
    let dataArray = [];
    this.state.MyTaskList.map((data) => {
      let mdData = [
    ...
    return (
       <>
        <Breadcrumb>
          ...
          <Breadcrumb.Item active>My Task : Approved List</Breadcrumb.Item>
        </Breadcrumb>
        <MUIDataTable
          title={""}
          data={dataArray}
