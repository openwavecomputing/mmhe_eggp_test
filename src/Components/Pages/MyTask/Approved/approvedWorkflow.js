...
class pendingWorkflow extends Component {
  ...
  componentDidMount() {
    this.props.dispatch({ type: "headerTitle", value: ((this.state.Referenceno) ? "Approved : WorkFlow - " + this.state.Referenceno : "Approved : WorkFlow")});
    let encryptedId = this.props.location.search;
    var CryptoJS = require("crypto-js");
    ...
    return (
       <>
        <Breadcrumb>
          ...
          <Breadcrumb.Item href={process.env.REACT_APP_ENV +"/mytask/approved/list"}>My Task : Approved List</Breadcrumb.Item>
