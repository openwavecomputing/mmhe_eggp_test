...
class rejectedList extends Component {
  ...
  componentDidMount() {
    this.props.dispatch({ type: 'headerTitle', value: "My Task : Rejected List" })
  ...
  viewRequest = (Referenceno) =>{
    ...
    window.location = process.env.REACT_APP_ENV +"/mytask/rejected/view?p=" + encodeURI(CryptoJS.AES.encrypt(JSON.stringify(Referenceno), process.env.REACT_APP_ENCRYPT_PASSWORD).toString());
  }

  workflowRequest = (Referenceno) =>{
    var CryptoJS = require("crypto-js");
    window.location = process.env.REACT_APP_ENV +"/mytask/rejected/workflow?p=" + encodeURI(CryptoJS.AES.encrypt(JSON.stringify(Referenceno), process.env.REACT_APP_ENCRYPT_PASSWORD).toString());
  }
  ...
  printRequest = (Referenceno) =>{
    var CryptoJS = require("crypto-js");
    window.location = process.env.REACT_APP_ENV +"/mytask/rejected/print?p=" + encodeURI(CryptoJS.AES.encrypt(JSON.stringify(Referenceno), process.env.REACT_APP_ENCRYPT_PASSWORD).toString());
  }
    ...
    return (
       <>
        <Breadcrumb>
          ...
          <Breadcrumb.Item active>My Task : Rejected List</Breadcrumb.Item>
        </Breadcrumb>
        <MUIDataTable
          title={""}
          data={dataArray}
