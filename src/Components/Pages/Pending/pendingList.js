...
class pendingList extends Component {
  ...
  componentDidMount() {
    this.props.dispatch({ type: 'headerTitle', value: "My Application : Pending List" })
    this.GetListData('Pending')
  }


  editRequest = () =>{
  //  alert(this.state.editidrefernceno)
    var CryptoJS = require("crypto-js");
    window.location = process.env.REACT_APP_ENV +"/pending/edit?p=" + encodeURI(CryptoJS.AES.encrypt(JSON.stringify(this.state.editidrefernceno), process.env.REACT_APP_ENCRYPT_PASSWORD).toString());
    this.setState({pendingeditPopup:false})
  }
  ...
  viewRequest = (Referenceno) =>{
    var CryptoJS = require("crypto-js");
    window.location = process.env.REACT_APP_ENV +"/pending/view?p=" + encodeURI(CryptoJS.AES.encrypt(JSON.stringify(Referenceno), process.env.REACT_APP_ENCRYPT_PASSWORD).toString());
  }

  workflowRequest = (Referenceno) =>{
    var CryptoJS = require("crypto-js");
    window.location = process.env.REACT_APP_ENV +"/pending/workflow?p=" + encodeURI(CryptoJS.AES.encrypt(JSON.stringify(Referenceno), process.env.REACT_APP_ENCRYPT_PASSWORD).toString());
  }
  ...
  cancelThisdraftedit = () =>{
    this.setState({  pendingeditPopup: false , editidrefernceno:"" })
  }

  edittodrafticon = (Referencenoid) => {
    this.setState({  pendingeditPopup: true, editidrefernceno:Referencenoid })
  }
  ...
  printRequest = (Referenceno) =>{
    var CryptoJS = require("crypto-js");
    window.location = process.env.REACT_APP_ENV +"/pending/print?p=" + encodeURI(CryptoJS.AES.encrypt(JSON.stringify(Referenceno), process.env.REACT_APP_ENCRYPT_PASSWORD).toString());
  }
         ...
         sort: true,
          customBodyRender: (value, tableMeta, updateValue) => {
            return (
              ...
              <ReactTooltip />              
             {(() => {
              if (value.IsEdit === 1 || value.IsEdit === "1") {
                return (
                      <img className="action-right-space mouse-pointer" style={{width: "25px"}} src={process.env.REACT_APP_ENV +"/assets/images/icons/action/edit-solid.svg"} data-tip="Edit" alt="Metro Flow Tree"  onClick={() => this.edittodrafticon(value.ggp_reference_no)} data-tip="Edit"/>
                      )
                    }
              })()}
    ...
    return (
       <>
        <Breadcrumb>
          ...
          <Breadcrumb.Item active>My Application : Pending List</Breadcrumb.Item>
        </Breadcrumb>
        <MUIDataTable
          title={""}
          data={dataArray}
        ...
        <Modal show={this.state.pendingeditPopup}>  
          <Modal.Header >Draft Confirmation</Modal.Header>  
          <Modal.Body>
            <div className="alert alert-danger" role="alert">
            Editing Pending GGP will move it to Draft section and you will need to submit for approval again. Do you confirm to proceed? 
            </div>
            <hr></hr>
            <div className="text-right">
              <Button className="btn btn-light"  onClick={this.cancelThisdraftedit}> No </Button> 
              <Button className="btn btn-primary-01 ml-3" onClick={this.editRequest}> Yes </Button>  
            </div>
          </Modal.Body>  
        </Modal>
        <FloatingButton />
