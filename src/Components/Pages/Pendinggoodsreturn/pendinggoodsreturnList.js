...
class pendinggoodsreturnList extends Component {
  constructor(props) {
    super(props);
  ...
  componentDidMount() {
    this.props.dispatch({ type: 'headerTitle', value: "My Application : Pending Goods Return List" })
    this.GetListData('Pending Goods Return')
  ...
  viewRequest = (Referenceno) =>{
    ...
    window.location = process.env.REACT_APP_ENV +"/pendinggoodsreturn/view?p=" + encodeURI(CryptoJS.AES.encrypt(JSON.stringify(Referenceno), process.env.REACT_APP_ENCRYPT_PASSWORD).toString());
  }

  workflowRequest = (Referenceno) =>{
    var CryptoJS = require("crypto-js");
    window.location = process.env.REACT_APP_ENV +"/pendinggoodsreturn/workflow?p=" + encodeURI(CryptoJS.AES.encrypt(JSON.stringify(Referenceno), process.env.REACT_APP_ENCRYPT_PASSWORD).toString());
  }
  ...
  downloadExcelFile = () =>{
      ...
      (async () => {
        await this.state.MyApplicationList.forEach((value,index) =>{
          let mdData = [
            ...
            value.requester,
            value.exit_time,
            ((value.submitted_date) ? moment(value.submitted_date).format("DD-MM-yyyy HH:mm:ss") : ""),            
            ((value.returndate) ? moment(value.returndate).format("DD-MM-yyyy") : ""),            
            value.ggpstatus,
          ];
          this.state.downloadExcelArray.push(mdData)
          return null;
    ...
    const filename = "pending_goods_return.xlsx";
  ...
  printRequest = (Referenceno) =>{
    var CryptoJS = require("crypto-js");
    window.location = process.env.REACT_APP_ENV +"/pendinggoodsreturn/print?p=" + encodeURI(CryptoJS.AES.encrypt(JSON.stringify(Referenceno), process.env.REACT_APP_ENCRYPT_PASSWORD).toString());
  }
    ...
    return (
       <>
        <Breadcrumb>
          ...
          <Breadcrumb.Item active>My Application : Pending Goods Return List</Breadcrumb.Item>
        </Breadcrumb>
        <MUIDataTable
          title={""}
          data={dataArray}
...
export default connect(mapStateToProps)(pendinggoodsreturnList);