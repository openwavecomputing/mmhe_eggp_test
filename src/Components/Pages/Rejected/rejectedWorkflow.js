...
class rejectedWorkflow extends Component {
  constructor(props) {
    super(props);
  ...
  componentDidMount() {
    this.props.dispatch({ type: "headerTitle", value: ((this.state.Referenceno) ? "Rejected : WorkFlow - " + this.state.Referenceno : "Rejected : WorkFlow")});
    let encryptedId = this.props.location.search;
    var CryptoJS = require("crypto-js");
    ...
    return (
       <>
        <Breadcrumb>
          ...
          <Breadcrumb.Item href={process.env.REACT_APP_ENV +"/rejected/list"}>My Application : Rejected List</Breadcrumb.Item>
...
export default connect(mapStateToProps)(rejectedWorkflow);