...
toast.configure();
class revisionEditApproverList extends Component {
  constructor(props) {
    super(props);
  ...
  componentDidMount() {
    this.props.dispatch({ type: "headerTitle", value: "Approver List" });
    let encryptedId = this.props.location.search;
    var CryptoJS = require("crypto-js");
  ...
  getStaffDetails = (id, name, Section, SectionName, Department, Division, Designation) =>{
    if(this.state.EditWorkflowid){
      this.setState({ EditRequesterId: id, EditRequesterName: name, getEmployeeFirstShow: false }, ()=> this.updateApprover())
    }else{
...
export default connect(mapStateToProps)(revisionEditApproverList);
