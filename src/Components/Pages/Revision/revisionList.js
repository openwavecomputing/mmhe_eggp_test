...
class revisionList extends Component {
  ...
  componentDidMount() {
    this.props.dispatch({ type: 'headerTitle', value: "My Application : Revision List" })
    this.GetListData('Revision')
  }
  ...
  viewRequest = (Referenceno) =>{
    ...
    window.location = process.env.REACT_APP_ENV +"/revision/view?p=" + encodeURI(CryptoJS.AES.encrypt(JSON.stringify(Referenceno), process.env.REACT_APP_ENCRYPT_PASSWORD).toString());
  }

  workflowRequest = (Referenceno) =>{
    var CryptoJS = require("crypto-js");
    window.location = process.env.REACT_APP_ENV +"/revision/workflow?p=" + encodeURI(CryptoJS.AES.encrypt(JSON.stringify(Referenceno), process.env.REACT_APP_ENCRYPT_PASSWORD).toString());
  }

  editRequest = (Referenceno) =>{
    var CryptoJS = require("crypto-js");
    window.location = process.env.REACT_APP_ENV +"/revision/edit?p=" + encodeURI(CryptoJS.AES.encrypt(JSON.stringify(Referenceno), process.env.REACT_APP_ENCRYPT_PASSWORD).toString());
  }

  cancelRequest = (Referenceno) =>{
    let postData = { 
  ...
  printRequest = (Referenceno) =>{
    var CryptoJS = require("crypto-js");
    window.location = process.env.REACT_APP_ENV +"/revision/print?p=" + encodeURI(CryptoJS.AES.encrypt(JSON.stringify(Referenceno), process.env.REACT_APP_ENCRYPT_PASSWORD).toString());
  }
       ...
       },
       {
        ...
        options: {
         ...
         sort: true,
          customBodyRender: (value, tableMeta, updateValue) => {
            return (
              <div className="badge badge-revision">{value}</div>
            );
          }
        }
       },
         ...
         sort: true,
          customBodyRender: (value, tableMeta, updateValue) => {
            return (
              ...
              <img className="action-right-space mouse-pointer" style={{width: "25px"}} src={process.env.REACT_APP_ENV +"/assets/images/icons/action/edit-solid.svg"} data-tip="Edit" alt="Metro Flow Tree"  onClick={() => this.editRequest(value)}/>
              ...
              <img className="action-right-space mouse-pointer" src={process.env.REACT_APP_ENV +"/assets/images/icons/action/ionic-md-close.svg"} data-tip="Cancel" alt="Close" onClick={() => this.cancelRequest(value)}/>
    ...
    return (
       <>
        <Breadcrumb>
          ...
          <Breadcrumb.Item active>My Application : Revision List</Breadcrumb.Item>
        </Breadcrumb>
        <MUIDataTable
          title={""}
          data={dataArray}
