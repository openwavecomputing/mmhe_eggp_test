...
class revisionView extends Component {
  ...
  componentDidMount() {
    ...
    this.props.dispatch({ type: 'headerTitle', value: "Revision : View" })
    this.getGGPRequest();
  }
  ...
  editFloatingIcon = () => {
    var CryptoJS = require("crypto-js");
    window.location = process.env.REACT_APP_ENV + "/revision/edit?p=" + encodeURI(CryptoJS.AES.encrypt(JSON.stringify(this.state.Referenceno), process.env.REACT_APP_ENCRYPT_PASSWORD).toString());
  }
  render() {
    if(this.state.loading){
        ...
        <>
            ...
            <button type="button" id="btn" className="btn btn-create-request" onClick={this.editFloatingIcon}>
            <i className="fa fa-edit p_5"></i>
            <span className="cr-text">&nbsp;Edit Request </span>
        </button>
        </>
        );
    }
...
export default connect(mapStateToProps)(revisionView);