...
class pendingconfirmgoodsreturnList extends Component {
  constructor(props) {
    super(props);
  ...
  componentDidMount() {
    this.props.dispatch({ type: 'headerTitle', value: "My Application : Pending Confirm Goods Return List" })
    this.GetListData('Pending Confirm Goods Return')
  ...
  viewRequest = (Referenceno) =>{    
    ...
    window.location = process.env.REACT_APP_ENV +"/pendingconfirmgoodsreturn/view?p=" + encodeURI(CryptoJS.AES.encrypt(JSON.stringify(Referenceno), process.env.REACT_APP_ENCRYPT_PASSWORD).toString());
  }

  workflowRequest = (Referenceno) =>{
    var CryptoJS = require("crypto-js");
    window.location = process.env.REACT_APP_ENV +"/pendingconfirmgoodsreturn/workflow?p=" + encodeURI(CryptoJS.AES.encrypt(JSON.stringify(Referenceno), process.env.REACT_APP_ENCRYPT_PASSWORD).toString());
  }
  ...
  downloadExcelFile = () =>{
      ...
      (async () => {
        await this.state.MyApplicationList.forEach((value,index) =>{
          let mdData = [
            ...
            value.requester,
            value.exit_time,
            ((value.submitted_date) ? moment(data.submitted_date).format("DD-MM-yyyy HH:mm:ss") : ""),
            ((value.returndate) ? moment(data.returndate).format("DD-MM-yyyy") : ""),           
            value.ggpstatus,
          ];
          this.state.downloadExcelArray.push(mdData)
          return null;
    ...
    const filename = "pending_confirm_goods_return.xlsx";
  ...
  printRequest = (Referenceno) =>{
    var CryptoJS = require("crypto-js");
    window.location = process.env.REACT_APP_ENV +"/pendingconfirmgoodsreturn/print?p=" + encodeURI(CryptoJS.AES.encrypt(JSON.stringify(Referenceno), process.env.REACT_APP_ENCRYPT_PASSWORD).toString());
  }

  confimRequest = (sReferenceno) =>{
    let postData = { 
        ReferenceNo: sReferenceno,
    }
    fetch(process.env.REACT_APP_WEB_SERVICE_URL + '/api/GGP/ConfirmGoodsInGGPRequest', {
    method: 'POST',
    headers: {
        'Content-Type': 'application/json',
        'Authorization': 'Bearer ' + this.props.getToken,
    ...
    .then(data => {
        ...
        this.props.history.push({ pathname: "/pendingconfirmgoodsreturn/list" });
        }
    })
  }
    ...
    return (
       <>
        <Breadcrumb>
          ...
          <Breadcrumb.Item active>My Application : Pending Confirm Goods Return List</Breadcrumb.Item>
        </Breadcrumb>
        <MUIDataTable
          title={""}
          data={dataArray}
...
export default connect(mapStateToProps)(pendingconfirmgoodsreturnList);