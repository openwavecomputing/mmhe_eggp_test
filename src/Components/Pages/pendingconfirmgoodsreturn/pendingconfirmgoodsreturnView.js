...
class pendingconfirmgoodsreturnView extends Component {
  constructor(props) {
    super(props);
  ...
  componentDidMount() {
    ...
    this.props.dispatch({ type: 'headerTitle', value: "Pending Confirm Goods Return : View" })
    this.getGGPRequest();
  }
  popupCancel = () =>{
  ...
  render() {
    if(this.state.loading){
        return (
            ...
            <Breadcrumb.Item href={process.env.REACT_APP_ENV +"/pendingconfirmgoodsreturn/list"}>Pending Confirm Goods Return</Breadcrumb.Item>
        ...
        <>
            ...
            <div className="text-right mb-3 mt-4">
                <button type="button" className="btn btn-success" onClick={() => this.showpop()} style={{marginRight: "10px"}}><i className="fa fa-check"></i> <span className="d-none d-sm-inline"> Confirm </span> </button>
                <button type="button" className="btn btn-outline-info" onClick={() => this.cancelRequest()}> <i class="fa fa-times"></i> <span className="d-none d-sm-inline">  Cancel </span></button>
            </div>
            </div>
            <Modal show={this.state.confirmpopup}>  
          <Modal.Header >Confirmation</Modal.Header>  
          <Modal.Body>
            <div className="alert alert-success" role="alert">
            Are you sure you want to confirm that goods have been returned?
            </div>
            <hr></hr>
            <div className="text-right">
              <Button className="btn btn-light"  onClick={this.popupCancel}> No </Button> 
              <Button className="btn btn-primary-01 ml-3" onClick={this.confimRequest}> Yes </Button>  
            </div>
          </Modal.Body>  
        </Modal>
        </>
...
export default connect(mapStateToProps)(pendingconfirmgoodsreturnView);